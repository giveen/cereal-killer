...
If you are bringing up a fresh local inference setup, start with the Gibson setup flow before launching the TUI:
...
This writes `.env` and `config/llama-swap/config.yaml`, can download the recommended HauhauCS Qwen3.6 quant, and appends setup results to `logs/setup.log` while validating NVIDIA, CUDA, llama-swap, Redis, and model artifacts.

### Option A: Docker Compose (recommended)

1. Generate local config and validate the host setup:
...
This writes `.env` and `config/llama-swap/config.yaml`, can download the recommended HauhauCS Qwen3.6 quant, and appends setup results to `logs/setup.log`.

2. Create environment file manually only if needed:

```bash
cp .env.example .env
```

3. Update `.env` and set at least `LLM_BASE_URL` to your running OpenAI-compatible endpoint.
...
1. `python scripts/setup/generate_config.py`
2. `./scripts/setup/gibson_check.sh /home/${USER}/models/gibson`
3. `make docker-build`
4. `make docker-up-init`
5. `make` (or `make run`)
...
- A valid `.env` and model setup generated by `python scripts/setup/generate_config.py`
- A passing `./scripts/setup/gibson_check.sh /home/${USER}/models/gibson`
...
`make run` executes the Gibson setup check before launching the TUI. If core preflight checks fail, the app opens in a setup-incomplete state.
...
- `make docker-up`: start Redis + SearXNG in the background
- `make docker-up-init`: run `make docker-up`, then run a full `sync-all` resync
- `make setup`: run config generation, then execute `./scripts/setup/gibson_check.sh`
- `make check` / `make check-env`: run `./scripts/setup/gibson_check.sh`
...
- Run full Gibson preflight check (appended to `logs/setup.log`):
